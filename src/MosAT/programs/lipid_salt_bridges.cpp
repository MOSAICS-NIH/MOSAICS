...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-crd_1",  p.param_file_name,            "Selection card with Lipid types + mapping atoms (crd)",       s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-crd_2",  p.lipid_charge_file_name,     "Selection card with lipid charge groups (crd)",               s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-crd_3",  p.protein_charge_file_name,   "Selection card with protein charge groups (crd)",             s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-lpsb",   p.lpsb_file_name,             "Output file with spatially resolved salt bridge count (dat)", s.world_rank, nullptr,      1);
    ...
    add_argument_mpi_i(argc,argv,"-test",   &p.b_test,                    "Print info for checking salt bridges? (0:no 1:yes)",          s.world_rank, nullptr,      0);
