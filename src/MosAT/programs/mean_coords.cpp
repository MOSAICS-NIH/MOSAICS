...
#include "MosAT/program_variables/pv_mean_coords.h"         //This has the variables specific to the analysis program
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                                           //
// This function adds the coords for the current frame to the long term sum. It also divides by the total    //
// number of frames so that this step is already done.                                                       //
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
void add_r(Trajectory &traj,system_variables &s,program_variables &p,rvec *r_avg)
{
    ...
    for(i=0; i<traj.atoms(); i++) //loop over system atoms
    {
        ...
        {
            r_avg[i][j] = r_avg[i][j] + traj.r[i][j]/(double)traj.get_ef_frames();
        }
    }
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                                           //
// Collect coords and get the average.                                                                       //
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
double finalize_analysis(Trajectory &traj,system_variables &s,program_variables &p,rvec *r_avg)
{
    int i = 0;  //standard variable used in loops
    int j = 0;  //standard variable used in loops
    ...
    for(i=0; i<traj.atoms(); i++) //loop over all atoms
    {
    ...
    //rank 0 now has the average coords. write out a pdb file
    if(s.world_rank == 0)
    {
        FILE *avg_file = fopen(p.avg_file_name.c_str(), "w");
        if(avg_file == NULL)
        {
            printf("failure opening %s. Make sure the file exists. \n",p.avg_file_name.c_str());
        }

        write_frame_pdb(traj.ibox,traj.atoms(),traj.atom_nr,traj.res_nr,traj.res_name,traj.atom_name,r_avg,traj.title,s.world_rank,&avg_file,traj.beta,traj.weight,traj.element,traj.chain_id,1);
...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-avg",    p.avg_file_name,              "Output file with time average coordinates (pdb)",             s.world_rank, s.cl_tags, nullptr,      1);
    ...
    check_extension_mpi(s.world_rank,"-avg",p.avg_file_name,".pdb");

    rvec *r_avg;             //data structure to hold the average coords
    //snew(r_avg,traj.atoms());
    r_avg = (rvec*)calloc(traj.atoms() , sizeof(rvec));
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    perf.log_time(finalize_analysis(traj,s,p,r_avg),"Fin Ana");
