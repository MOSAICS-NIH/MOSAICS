...
#include "MosAT/program_variables/pv_inter_leaflet_contacts.h" //This has the variables specific to the analysis program
...
void get_contacts(Trajectory &traj,system_variables &s,program_variables &p,Param &param_1,Param &param_2,Grid &lfc,iv1d &contacts,dv1d &contacts_frame)
{
    ...
    if(p.b_stdev == 1)
    {
        lfc.write_frame(traj.get_frame_global());
    }
}
...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
double finalize_analysis(Trajectory &traj,system_variables &s,program_variables &p,Grid &lfc,iv1d &contacts,dv1d &contacts_frame)
{
    int i = 0; //standard variable used in loops
    int j = 0; //standard variable used in loops 
    ...
    lfc.get_stdev(p.b_stdev,p.b_clean,traj);
    ...
    if(s.world_rank == 0)
    {
        ...
        string frame_file_name = add_tag(p.lfc_file_name,"_contacts_frame");
...
int main(int argc, const char * argv[]) 
{
    ...
    s.program_name = "Inter-Leaflet Contacts";
    ...
    add_argument_mpi_s(argc,argv,"-lfc",    p.lfc_file_name,              "Output file with spatially resolved ILC (dat)",               s.world_rank, nullptr,      1);
    ...
    check_extension_mpi(s.world_rank,"-lfc",p.lfc_file_name,".dat");
    ...
    //create a grid to hold interleaflet contacts
    Grid lfc;

    //get the grid dimensions
    lfc.get_dim(p.box_x,p.box_y,traj.ibox,p.APS);

    //set the output file name for grid
    lfc.set_output(p.lfc_file_name,p.out_data_format);
