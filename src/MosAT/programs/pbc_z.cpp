...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_d(argc,argv,"-cutoff", &p.cutoff,                    "Percentage of box height that an atom must moved between frames to be counted as a jump (0 to 1)", s.world_rank, s.cl_tags, nullptr,      1);
    ...
    add_argument_mpi_s(argc,argv,"-record", p.record_file_name,           "Output data file with the jump records (dat)",                                                     s.world_rank, s.cl_tags, &p.b_record,  0);
