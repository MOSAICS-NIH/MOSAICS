...
#include "MosAT/program_variables/pv_mosaics_to_charmm_card.h"       //This has the variables specific to the analysis program
...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
void write_card(Trajectory &traj,system_variables &s,program_variables &p,Index &param)
{
    int     i = 0;     //standard variable used in loops
    int     j = 0;     //standard variable used in loops

    if(p.b_crd == 1) //rename select residues
    {
        for(i=0; i<param.index_s.size(); i+=3) //loop over index items
        {
            string name = param.index_s[i+0];
            int start   = param.index_i[i+1];
            int end     = param.index_i[i+2];

            for(j=0; j<traj.atoms(); j++)
            {
                if(traj.res_nr[j] >= start && traj.res_nr[j] <= end)
                {
                    traj.res_name[j] = name; 
                }
            }
        }
    }
    ...
    //open pdb file
    FILE *card_file;
    card_file = fopen(p.card_file_name.c_str(), "w");
    if(card_file == NULL)
    {
        printf("failure opening %s. Make sure the file exists. \n",p.card_file_name.c_str());
    }
    else
    {
        fprintf(card_file,"%10d  EXT\n",traj.atoms());
        for(i=0; i<traj.atoms(); i++) //loop over system atoms
        {
            //convert coords to angstorm too!
            fprintf(card_file,"%10d%10d  %-6s    %-6s%15.3f%20.3f%20.3f%16.5f%10.5f%16.5f\n",traj.atom_nr[i],traj.res_nr[i],traj.res_name[i].c_str(),traj.atom_name[i].c_str(),10.0*traj.r[i][0],10.0*traj.r[i][1],10.0*traj.r[i][2],0.0,0.0,0.0);
        }
        fclose(card_file);
    }
}
...
int main(int argc, const char * argv[]) 
{
    ...
    s.program_name = "MOSAICS to CHARMM Card";
    ...
    add_argument_mpi_s(argc,argv,"-card",   p.card_file_name,             "Output CHARMM card with atomic coordinates",                      s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-crd",    p.param_file_name,            "Selection card with lipid atom info (crd)",                       s.world_rank, s.cl_tags, &p.b_crd,     0);
    ...
    check_extension_mpi(s.world_rank,"-card",p.card_file_name,".crd");
    if(p.b_crd == 1)
    { 
        check_extension_mpi(s.world_rank,"-crd",p.param_file_name,".crd");
    }

    //create index for residue types to rename
    Index param;
