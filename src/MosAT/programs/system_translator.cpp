...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-traj_b", p.traj_2_file_name,           "Align -traj to to match me (xtc, trr, pdb, gro)",             s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-ref_b",  p.ref_b_file_name,            "Refference file for -traj_b (pdb, gro)",                      s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-ind",    p.index_file_name,            "Atom selection for -traj (ndx)",                              s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-ind_b",  p.index_b_file_name,          "Atom selection for -traj_b (ndx)",                            s.world_rank, s.cl_tags, nullptr,      1);
