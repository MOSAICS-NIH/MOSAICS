...
#include "MosAT/program_variables/pv_apl.h"                 //This has the variables specific to the analysis program
...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
void area(Trajectory &traj,Trajectory &traj_v,system_variables &s,program_variables &p,Index &param1,Param &param2,Grid &apl,dv1d &area_measurements)
{
    ...
    {
        //set output filename
        string voronoi_file_name = add_tag(p.apl_file_name,to_string(traj.get_frame_global())); 
    ...
    if(p.b_stdev == 1)
    {
        apl.write_frame(traj.get_frame_global());
    }
}
...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
double finalize_analysis(Trajectory &traj,system_variables &s,program_variables &p,Grid &apl,dv1d &area_measurements)
{
    ...
    apl.get_stdev(p.b_stdev,p.b_clean,traj);
    ...
    {    
        ...
        string histo_file_name = add_tag(p.apl_file_name,"_histo");
...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-traj_v", p.traj_voro_file_name,        "Input trajectory file for constructing voronoi diagrams (xtc, trr, pdb, gro)", s.world_rank, nullptr,      1);
    ...
    add_argument_mpi_s(argc,argv,"-apl",    p.apl_file_name,              "Output file with spatially resolved area per lipid (dat)",                     s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-crd_1",  p.param1_file_name,           "Selection card with target lipid types and mapping atoms (crd)",               s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-crd_2",  p.param2_file_name,           "Selection card with voronoi diagram lipid types (crd)",                        s.world_rank, nullptr,      1);
    ...
    add_argument_mpi_s(argc,argv,"-pf_pdb", p.pf_pdb_file_name,           "PDB file with protein marked (pdb)",                                           s.world_rank, &p.b_pf_pdb,  0);
    ...
    add_argument_mpi_d(argc,argv,"-c_dist", &p.c_dist,                    "Distance cutoff for counting protein atoms in voronoi diagram (nm)",           s.world_rank, nullptr,      1);
    add_argument_mpi_i(argc,argv,"-voro",   &p.b_voronoi,                 "Write single frame voronoi files? (0:no 1:yes)",                               s.world_rank, nullptr,      0);
    add_argument_mpi_d(argc,argv,"-voro_r", &p.voro_stamp_rad,            "Stamping radius used when computing Voronoi diagrams (nm)",                    s.world_rank, nullptr,      0);
    add_argument_mpi_d(argc,argv,"-bin",    &p.bin_width,                 "Bin width for histogram (nm^2)",                                               s.world_rank, nullptr,      0);
    ...
    check_extension_mpi(s.world_rank,"-apl",p.apl_file_name,".dat");
    ...
    apl.get_dim(p.box_x,p.box_y,traj.ibox,p.APS);

    //set the output file name for grid
    apl.set_output(p.apl_file_name,p.out_data_format);
