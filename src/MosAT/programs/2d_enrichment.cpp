...
#include "MosAT/program_variables/pv_2d_enrichment.h"       //This has the variables specific to the analysis program
...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
void enrichment(Trajectory &traj,system_variables &s,program_variables &p,Param &param_1,Param &param_2,Grid &rho1,Grid &rho2,Grid &rho_t)
{
    ...
    if(p.print_stride > 0)
    {
        ...
        {
            rho1.write_frame(traj.get_frame_global());
            rho2.write_frame(traj.get_frame_global());
            rho_t.write_frame(traj.get_frame_global());
        }
    }
}
...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
double finalize_analysis(Trajectory &traj,system_variables &s,program_variables &p,Grid &rho1,Grid &rho2,Grid &rho_t,Grid &enrich)
{
    int i = 0;                              //standard variable used in loops
    int j = 0;                              //standard variable used in loops
    ...
    //collect rho1 and rho2 from all ranks
    rho1.collect_grid();
    rho2.collect_grid();
    rho_t.collect_grid();
    ...
    if(s.world_rank == 0)
    {
        ...
        //compute enrichment
        for(i=0; i<enrich.num_x(); i++) //loop over x-dimension
        {
            for(j=0; j<enrich.num_y(); j++) //loop over y-dimension
            {
                 if(rho2.grid[j][i] > 0) //not division by zero
                 {
                     double ratio = rho1.grid[j][i]/rho2.grid[j][i];
                     enrich.grid[j][i] = ((ratio - p.bulk)/p.bulk)*100;
                }
                else //division by zero
                {
                    enrich.grid[j][i] = 999999999999;
                }
            }
        }
...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-crd_1",  p.param_1_file_name,          "Selection card for lipids A (crd)",                           s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-crd_2",  p.param_2_file_name,          "Selection card for lipids B (crd)",                           s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-enrich", p.enrich_file_name,           "Output file with spatially resolved enrichment factor (dat)", s.world_rank, nullptr,      1);
    ...
    add_argument_mpi_i(argc,argv,"-ps",     &p.print_stride,              "Print single frame stamping data every nth frame (0:off)",    s.world_rank, nullptr,      0);
    ...
    rho1.get_dim(p.box_x,p.box_y,traj.ibox,p.APS);
    rho2.get_dim(p.box_x,p.box_y,traj.ibox,p.APS);
    rho_t.get_dim(p.box_x,p.box_y,traj.ibox,p.APS);
    enrich.get_dim(p.box_x,p.box_y,traj.ibox,p.APS);
    ...
    rho1.set_output(rho1_file_name,p.out_data_format);
    rho2.set_output(rho2_file_name,p.out_data_format);
    rho_t.set_output(rho_t_file_name,p.out_data_format);
    enrich.set_output(p.enrich_file_name,p.out_data_format);
