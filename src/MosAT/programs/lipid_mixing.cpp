...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-mix",    p.mix_file_name,              "Output data file with mixing fraction and solvation number (dat)",     s.world_rank, s.cl_tags, nullptr,      1);
    ...
    add_argument_mpi_i(argc,argv,"-mix_s",  &p.mix_stride,                "How often to print mixing data to output file (steps)",                s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_d(argc,argv,"-dt",     &p.ef_dt,                     "Effective time step between anayzed frames (counting stide, ps) ",     s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-m1",     p.map_1,                      "Name of mapping atom 1 ",                                              s.world_rank, s.cl_tags, &p.b_m1,      0);
    add_argument_mpi_s(argc,argv,"-m2",     p.map_2,                      "Name of mapping atom 2 ",                                              s.world_rank, s.cl_tags, &p.b_m2,      0);
    add_argument_mpi_d(argc,argv,"-APS",    &p.APS,                       "Area per grid box (nm^2)",                                             s.world_rank, s.cl_tags, &p.b_APS,     0);
    add_argument_mpi_d(argc,argv,"-r",      &p.radius,                    "Radius of the target atom (nm)",                                       s.world_rank, s.cl_tags, &p.b_r,       0);
    add_argument_mpi_d(argc,argv,"-cutoff", &p.cutoff,                    "Cutoff for excluding data (chi)",                                      s.world_rank, s.cl_tags, &p.b_cutoff,  0);
    ...
    add_argument_mpi_d(argc,argv,"-freq",   &p.window_cutoff,             "Noise filter significance threshold (omega)",                          s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_i(argc,argv,"-w_bind", &p.b_report_binding_events,   "Report binding events for each lipid's solvation shell? (0:no 1:yes)", s.world_rank, s.cl_tags, nullptr,      0);
