...
#include "MosAT/program_variables//pv_pdb_editor.h"            //This has the variables specific to the analysis program
...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
void set_factor(Trajectory &traj,system_variables &s,program_variables &p,iv1d &custom_sel)
{
    ...
    for(i=0; i<traj.atoms(); i++) //loop over system atoms
    {
        ...
        {   
            ...
            if(p.b_occupancy == 1)
            {
                traj.weight[i] = p.occupancy; 
            }
            if(p.b_chain_id == 1)
            {
                traj.chain_id[i] = p.chain_id_string[0];
	    }
            if(p.b_element == 1)
            {
...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-ele",    p.element,                    "Desired value for element",                                   s.world_rank, s.cl_tags, &p.b_element,   0);
    ...
    add_argument_mpi_i(argc,argv,"-leaf",   &p.leaflet,                   "Which leaflet? (0:both 1:upper 2:lower)",                     s.world_rank, s.cl_tags, nullptr,        0);
    ...
    //check that a single character was entered for chain id 
    if(p.b_chain_id == 1)
    {
        if(p.chain_id_string.length() > 1)
        {
            if(s.world_rank == 0)
            {
                printf("Argument entered for -chain with more than one character. This is not allowed. \n");
            }
            MPI_Finalize();
            exit(EXIT_SUCCESS);
        }
    ...
    for(traj.current_frame=0; traj.current_frame<traj.get_num_frames(); traj.current_frame++)
    {
        ...
        set_factor(traj,s,p,custom_sel);
