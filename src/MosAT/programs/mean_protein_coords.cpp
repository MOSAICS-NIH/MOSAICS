...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-mpc",    p.mpc_file_name,              "Output file with time average protein coords (pdb)",           s.world_rank, s.cl_tags, nullptr,          1);
    add_argument_mpi_s(argc,argv,"-pf_pdb", p.pf_pdb_file_name,           "PDB file with protein indicated (pdb)",                        s.world_rank, s.cl_tags, &p.print_prot_pdb,0);
    ...
    add_argument_mpi_i(argc,argv,"-dist",   &p.b_mean_dist,               "Compute the average dist from the average coords (0:no,1:yes)",s.world_rank, s.cl_tags, nullptr,          0);
