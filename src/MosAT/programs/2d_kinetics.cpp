...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-crd",    p.param_file_name,            "Selection card with target lipids for voronoi diagrame + mapping atoms (crd)", s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-k",      p.k_file_name,                "Output filename used to derive filenames for kinetics data (.dat)",            s.world_rank, nullptr,      1);
    ...
    add_argument_mpi_d(argc,argv,"-r",      &p.radius,                    "Stamping radius when computing the sample count (nm)",                         s.world_rank, nullptr,      1);
    ...
    add_argument_mpi_d(argc,argv,"-dt",     &p.delta_t,                   "Effective time step between trajectory frames analyzed (ps) ",                 s.world_rank, nullptr,      1);
