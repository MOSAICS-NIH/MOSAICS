...
#include "MosAT/program_variables/pv_traj_time.h"           //This has the variables specific to the analysis program
...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_d(argc,argv,"-dt",     &p.time_step,                 "Time between trajectory frames (ps)",                         s.world_rank, s.cl_tags, nullptr,      1);
    ...
    for(traj.current_frame=0; traj.current_frame<traj.get_num_frames(); traj.current_frame++)
    {
        ...
        traj.time = traj.get_frame_global()*p.time_step;
