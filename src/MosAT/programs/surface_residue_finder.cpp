...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-histo",  p.histo_file_name,            "Output file with surface atoms histogram (dat)",                s.world_rank, nullptr,      1);
    add_argument_mpi_i(argc,argv,"-bin",    &p.lsq_ref,                   "Bin width for the surface atoms histogram (atoms, int)",        s.world_rank, nullptr,      0);
    add_argument_mpi_s(argc,argv,"-s_pdb",  p.surface_pdb_file_name,      "Output file with surface residues indicated by B-factor (pdb)", s.world_rank, nullptr,      1);
