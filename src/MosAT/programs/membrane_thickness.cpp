...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-crd",    p.param_file_name,            "Selection card with lipid types and center atoms (crd)",      s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-thk",    p.thk_file_name,              "Output grid data with <thickness> (dat)",                     s.world_rank, nullptr,      1);
    ...
    add_argument_mpi_d(argc,argv,"-xy",     &p.xy_cutoff,                 "How close in xy before counting lipid in min dist (nm)",      s.world_rank, nullptr,      1);
    add_argument_mpi_d(argc,argv,"-width",  &p.bin_width,                 "Width of bins for free energy profile (nm)",                  s.world_rank, nullptr,      1);
    add_argument_mpi_d(argc,argv,"-dump_u", &p.upper_cutoff,              "Dump frame with thickness below this value (nm)",             s.world_rank, nullptr,      0);
    add_argument_mpi_d(argc,argv,"-dump_l", &p.lower_cutoff,              "Dump frame with thickness above this value (nm)",             s.world_rank, nullptr,      0);
    add_argument_mpi_d(argc,argv,"-temp",   &p.temp,                      "Temperature of the simulation (k)",                           s.world_rank, nullptr,      1);
    add_argument_mpi_i(argc,argv,"-all",    &p.count_all,                 "Use all lipids from opposing leaflet? (0:no 1:yes)",          s.world_rank, nullptr,      0);
