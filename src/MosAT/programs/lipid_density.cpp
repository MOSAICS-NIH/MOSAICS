...
#include "MosAT/program_variables/pv_lipid_density.h"       //This has the variables specific to the analysis program
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                                          //
// This function computes the lipid density over a frame                                                    //
//                                                                                                          //
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
void density(Trajectory &traj,system_variables &s,program_variables &p,Param &param,dv2d &rho)
{
    ...
    for(i=0; i<traj.target_leaflet.size(); i++) //loop over target leaflet atoms
    {
        for(j=0; j<param.main_size_y(); j++) //loop over lipid types 
        {
            ...
            {
                for(k=0; k<param.sec_size_y(j); k++) //loop over target atoms
                {
                    if(strcmp(traj.atom_name[traj.target_leaflet[i]-1].c_str(), param.param_sec_s[j][k][0].c_str()) == 0) //atom type is correct
                    {
                        double rx = traj.r[traj.target_leaflet[i]-1][0];  //mapping atom x-coord
                        double ry = traj.r[traj.target_leaflet[i]-1][1];  //mapping atom y-coord

                        deposit_lipid_density(rx,ry,p.radius,p.cell_size,rho,traj.get_ef_frames(),param.sec_size_y(j));
                    }
                }
            }
        }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                                           //
// Collect density and compute average                                                                       //
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
double finalize_analysis(Trajectory &traj,system_variables &s,program_variables &p,dv2d &rho)
{
    int i             = 0;                               //standard variable used in loops
    int j             = 0;                               //standard variable used in loops
    int k             = 0;                               //standard variable used in loops
    ...
    double num_lipids = 0;
    iv2d nan(p.num_g_y,iv1d(p.num_g_x,0));
    ...
    //collect rho from all ranks
    collect_grid_d(s.world_size,s.world_rank,rho);

    //normalize rho and write the rho to file
    if(s.world_rank == 0)
    {
        double avg_rho = get_average_rho(rho);  

        exclude_insignificant_data(p.cutoff,avg_rho,rho,nan);

        write_grid_to_file_d(p.out_data_format,p.cell_size,p.rho_file_name,nan,rho);
    }

    //check if summing over the density gives the number of lipids in the box
    if(s.world_rank == 0)
    {
        for(i=0; i<p.num_g_x; i++) //loop over x
        {
            ...
            {
                num_lipids = num_lipids + rho[j][i];
            }
        }

        printf("Summing over rho estimates there to be %f lipids in the box \n",num_lipids);
    }
...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-crd",    p.param_file_name,            "Selection card with lipid types and target atoms (crd)",      s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-rho",    p.rho_file_name,              "Output file holding grid with lipid density (dat)",           s.world_rank, nullptr,      1);
    ...
    check_extension_mpi(s.world_rank,"-rho",p.rho_file_name,".dat");
    ...
    //now we have the initial box size we set up the grid
    ...
    dv2d rho(p.num_g_y,dv1d(p.num_g_x,0.0));                     //This holds the lipid density for each grid point
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //collect lipid density from mpi processes and compute average
    perf.log_time(finalize_analysis(traj,s,p,rho),"Fin Ana");
