...
#include "MosAT/program_variables/pv_nearest_neighbors.h"   //This has the variables specific to the analysis program
...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
void neighbors(Trajectory &traj,system_variables &s,program_variables &p,Param &param_1,Param &param_2,Grid &nbr)
{
            ...
            if(strcmp(traj.res_name[min_1].c_str(), param_1.param_main_s[j][0].c_str()) == 0) //lipid type is correct
            {
                ...
                for(l=min_1; l<=max_1; l++) //loop over current residue
                {
                    if(strcmp(traj.atom_name[l].c_str(), param_1.param_main_s[j][1].c_str()) == 0) //atom is mapping atom 1
                    {
                        ...
                        nbr.stamp(rx,ry,p.radius,neighbors);
                    }
    ...
    if(p.b_stdev == 1)
    {
        nbr.write_frame(traj.get_frame_global());
    }
}
...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
double finalize_analysis(Trajectory &traj,system_variables &s,program_variables &p,Grid &nbr)
{
    ...
    nbr.get_stdev(p.b_stdev,p.b_clean,traj);
...
int main(int argc, const char * argv[]) 
{
    ...
    s.program_name = "Nearest Neighbors";
    ...
    add_argument_mpi_s(argc,argv,"-nbrs",   p.nbrs_file_name,             "Output grid with spatially resolved nearest neighbors count (dat)", s.world_rank, nullptr,      1);
    ...
    add_argument_mpi_d(argc,argv,"-l_rad",  &p.local_rad,                 "Radius of sphere for counting neighbors (nm)",                      s.world_rank, nullptr,      1);
    ...
    check_extension_mpi(s.world_rank,"-nbrs",p.nbrs_file_name,".dat");
    ...
    nbr.get_dim(p.box_x,p.box_y,traj.ibox,p.APS);

    //set the output file name for grid
    nbr.set_output(p.nbrs_file_name,p.out_data_format);
