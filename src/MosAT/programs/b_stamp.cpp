...
#include "MosAT/program_variables/pv_b_stamp.h"              //This has the variables specific to the analysis program
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                                           //
// This function stampls the b-factor from traj_b to the first traj                                          //
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
void b_stamp(Trajectory &traj,system_variables &s,program_variables &p,Trajectory &traj_b)
{
    int i = 0;
    ...
    for(i=0; i<traj.atoms(); i++) //loop over the system atoms
    {
        traj.beta[i] = traj_b.beta[i];
         ...
         if(strcmp(traj.atom_name[i].c_str(), traj_b.atom_name[i].c_str()) != 0) //mismatch between trajectory files
         {
             if(s.world_rank == 0)
             {
                 printf("There appears to be a mismatch between the trajectory files. \n");
                 printf(" %20s %9d atom name %5s \n","(-traj) atom id",i,traj.atom_name[i].c_str());
                 printf(" %20s %9d atom name %5s \n","(-traj_b) atom id",i,traj_b.atom_name[i].c_str());
             }
             MPI_Finalize();
             exit(EXIT_SUCCESS);
         }
...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-traj",   p.in_file_name,               "Input trajectory file with desired structure (xtc, trr, pdb, gro)",                  s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-ref",    p.ref_file_name,              "Refference file (pdb, gro)",                                                         s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-o",      p.out_file_name,              "Output trajectory file with desired structure and B values (xtc, trr, pdb, gro)",    s.world_rank, s.cl_tags, &p.b_print,   0);
    ...
    add_argument_mpi_s(argc,argv,"-traj_b", p.traj_2_file_name,           "Input trajectory file with desired B values (xtc, trr, pdb, gro)",                   s.world_rank, s.cl_tags, nullptr,      1);
