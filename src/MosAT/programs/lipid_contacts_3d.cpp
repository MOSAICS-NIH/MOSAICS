...
void lip_cont(Trajectory &traj,system_variables &s,program_variables &p,Param &param,Grid_3d &lcont,Grid_3d &lmol,iv1d &custom_sel,iv1d &target_atoms)
{
            ...
            if(strcmp(traj.res_name[min_l].c_str(), param.param_main_s[j][0].c_str()) == 0) //lipid type is correct
            {
                ...
                for(k=min_l; k<=max_l; k++) //loop over current lipid atoms
                {
                    if(strcmp(traj.atom_name[k].c_str(), param.param_main_s[j][1].c_str()) == 0) //mapping atom 1
                    {
                        ...
                        lmol.stamp(hx,hy,hz,p.radius,molecules);
                    }
    ...
    if(p.b_stdev == 1)
    {
        ...
        lmol.write_frame(traj.get_frame_global(),p.ex_val);
    }
}
...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
double finalize_analysis(Trajectory &traj,system_variables &s,program_variables &p,Grid_3d &lcont,Grid_3d &lmol)
{
    ...
    if(s.world_rank == 0)
    {
        ...
        lmol.write_grid(p.ex_val);
        lmol.write_rho(p.ex_val);
    }

    //compute standard deviation
    lcont.get_stdev(p.b_stdev,p.b_clean,traj,p.ex_val);
    lmol.get_stdev(p.b_stdev,p.b_clean,traj,p.ex_val);
...
int main(int argc, const char * argv[]) 
{
    ...
    lmol.get_dim(p.box_x,p.box_y,p.box_z,traj.ibox,p.APS);
