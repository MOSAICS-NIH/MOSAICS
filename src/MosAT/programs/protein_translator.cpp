...
#include "MosAT/program_variables/pv_protein_translator.h"  //This has the variables specific to the analysis program
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                                           //
// This function translates the system so the center of the two com groups lies on the target                //
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
void shift_atoms(Trajectory &traj,system_variables &s,program_variables &p)
{
    int i = 0;  //standard variable used in loops

    //compute shifting amounts
    double dx = p.com2[0] + 0.5*(p.com1[0] - p.com2[0]) - p.target[0];
    double dy = p.com2[1] + 0.5*(p.com1[1] - p.com2[1]) - p.target[1];
    double dz = p.com2[2] + 0.5*(p.com1[2] - p.com2[2]) - p.target[2];
    ...
    for(i=0; i<traj.atoms(); i++) //loop over system atoms
    {
        ...
        //put atom back in box if moved outside
        if(traj.r[i][0] < 0)
        {
            traj.r[i][0] = traj.r[i][0] + traj.box[XX][XX];
        }
        else if(traj.r[i][0] > traj.box[XX][XX])
        {
            traj.r[i][0] = traj.r[i][0] - traj.box[XX][XX];
        }
        ...
        if(traj.r[i][1] < 0)
        {
            traj.r[i][1] = traj.r[i][1] + traj.box[YY][YY];
        }
        else if(traj.r[i][1] > traj.box[YY][YY])
        {
            traj.r[i][1] = traj.r[i][1] - traj.box[YY][YY];
        }
        ...
        if(traj.r[i][2] < 0)
        {
            traj.r[i][2] = traj.r[i][2] + traj.box[ZZ][ZZ];
        }
        else if(traj.r[i][2] > traj.box[ZZ][ZZ])
        {
            traj.r[i][2] = traj.r[i][2] - traj.box[ZZ][ZZ];
        }
    }
...
int main(int argc, const char * argv[]) 
{
    ...
    s.program_name = "Protein Translator";
    ...
    add_argument_mpi_d(argc,argv,"-x",      &p.target[0],                 "Translate the x-component of center 3 to this coordinate (nm)", s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_d(argc,argv,"-y",      &p.target[1],                 "Translate the y-component of center 3 to this coordinate (nm)", s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_d(argc,argv,"-z",      &p.target[2],                 "Translate the z-component of center 3 to this coordinate (nm)", s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-n1",     p.index1_file_name,           "Atom selection for center 1 (crd)",                             s.world_rank, s.cl_tags, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-n2",     p.index2_file_name,           "Atom selection for center 2 (crd)",                             s.world_rank, s.cl_tags, nullptr,      1);
    ...
    check_extension_mpi(s.world_rank,"-n1",p.index1_file_name,".crd");
    check_extension_mpi(s.world_rank,"-n2",p.index2_file_name,".crd");
    ...
    index1.get_index(p.index1_file_name);
    index2.get_index(p.index2_file_name);
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for(traj.current_frame=0; traj.current_frame<traj.get_num_frames(); traj.current_frame++)
    {
        ...
        p.com1 = traj.com_i(index1.get_column_i(1,0));
        p.com2 = traj.com_i(index2.get_column_i(1,0));

        shift_atoms(traj,s,p);
