...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
void p2(Trajectory &traj,system_variables &s,program_variables &p,Param &param,Grid &order,Grid &theta)
{
            ...
            if(strcmp(traj.res_name[min].c_str(), param.param_main_s[j][0].c_str() ) == 0) //lipid type is correct
            {
                ...
                for(k=min; k<=max; k++) //loop over current residue atoms
                {
                    if(strcmp(traj.atom_name[k].c_str(), param.param_main_s[j][1].c_str() ) == 0) //mapping atom 1
                    {
                        ...
                        order.stamp(hx,hy,p.radius,avg_p2);
                        theta.stamp(hx,hy,p.radius,avg_theta);
                    }
...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-p2",     p.p2_file_name,               "Output grid data with the spatially resolved time average (dat)",   s.world_rank, s.cl_tags, nullptr,      1);
