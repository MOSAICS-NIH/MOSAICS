...
#include "MosAT/program_variables/pv_lipid_orientation_3d.h"//This has the variables specific to the analysis program
...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
void lip_orientation(Trajectory &traj,system_variables &s,program_variables &p,Index &pairs,Grid_3d &sum_x,Grid_3d &sum_y,Grid_3d &sum_z)
{
    ...
    double hz = 0;                              //mapping atom z coord
                            ...
                            if(strcmp(traj.atom_name[l].c_str(), pairs.index_s[j+2].c_str()) == 0) //atom is tail 
                            {
                                for(m=min; m<=max; m++) //loop over current residue atoms
                                {
                                    if(strcmp(traj.atom_name[m].c_str(), pairs.index_s[j+3].c_str()) == 0) //atom is mapping atom
                                    {
                                        ...
                                        sum_x.stamp(hx,hy,hz,p.radius,dx);
                                        sum_y.stamp(hx,hy,hz,p.radius,dy);
                                        sum_z.stamp(hx,hy,hz,p.radius,dz);
                                    }
                                }
                            }
                        }
...
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
double finalize_analysis(Trajectory &traj,system_variables &s,program_variables &p,Grid_3d &sum_x,Grid_3d &sum_y,Grid_3d &sum_z,Grid_3d &p2,Grid_3d &phi,Grid_3d &theta,Grid_3d &dist)
{
    int    i  = 0;                //standard variable used in loops
    int    j  = 0;                //standard variable used in loops
    int    k  = 0;                //standard variable used in loops
    ...
    if(s.world_rank == 0)
    {
        ...
        for(j=0; j<sum_x.num_z(); j++) //loop over z-dimension
        {
            for(k=0; k<sum_x.num_y(); k++) //loop over y-dimension
            {
                for(l=0; l<sum_x.num_x(); l++) //loop over x-dimension
                {
                    if(sum_x.rho[j][k][l] > 0) //data present
                    {
                        //transfer vector to array for use with dot function
                        double vec[3];
                        vec[0] = sum_x.grid[j][k][l];
                        vec[1] = sum_y.grid[j][k][l];
                        vec[2] = sum_z.grid[j][k][l];
                        ...
                        theta.grid[j][k][l] = acos(cos_theta)*(180/pi); //angle in degrees

                        //compute p2
                        p2.grid[j][k][l] = 0.5*( (3*(cos_theta)*(cos_theta)) - 1);
                        ...
                        phi.grid[j][k][l]   = acos(cos_phi)*(180/pi);

                        //if orientation vector is in quadrant 2 or 3 the anlgel (greater than 180 from y) should be corrected 
                        if(vec[0] < 0)
                        {
                            phi.grid[j][k][l] = 2*(180 - phi.grid[j][k][l]) + phi.grid[j][k][l];
                        }

                        //compute r
                        dist.grid[j][k][l] = sqrt(vec[0]*vec[0] + vec[1]*vec[1] + vec[2]*vec[2]);
                    }
                }
            }
        }
        ...
        sum_x.set_output(x_file_name);
        sum_y.set_output(y_file_name);
        sum_z.set_output(z_file_name);
        p2.set_output(p2_file_name);
        phi.set_output(phi_file_name);
        theta.set_output(theta_file_name);
        dist.set_output(dist_file_name);

        //write data to the output files 
        sum_x.write_grid(p.ex_val);
        sum_y.write_grid(p.ex_val);
        sum_z.write_grid(p.ex_val);
        p2.write_grid(p.ex_val);
        phi.write_grid(p.ex_val);
        theta.write_grid(p.ex_val);
        dist.write_grid(p.ex_val);
        p2.write_rho(p.ex_val);
    }
    MPI_Barrier(MPI_COMM_WORLD);
...
int main(int argc, const char * argv[]) 
{
    ...
    s.program_name = "Lipid Orientation 3D";
    ...
    add_argument_mpi_s(argc,argv,"-pairs",  p.pairs_file_name,            "Selection card with orientational vector definitions (crd)",  s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-p2",     p.p2_file_name,               "Output file with spatially resolved tilt angle (dx)",         s.world_rank, nullptr,      1);
    ...
    sum_x.get_dim(p.box_x,p.box_y,p.box_z,traj.ibox,p.APS);
    if(s.world_rank == 0) //attempt to estimate requirements befor a crash
    {
        float mem_d = 4.0*(float)sum_x.num_x()*(float)sum_x.num_y()*(float)sum_x.num_z()*8.0;
        float mem_i = 2.0*(float)sum_x.num_x()*(float)sum_x.num_y()*(float)sum_x.num_z()*4.0;
        float mem_t = 7.0*(mem_d + mem_i);
        printf("Estimated memory to hold the grid: %f (MB) \n\n",mem_t/1000000.0);
    }
    sum_y.get_dim(p.box_x,p.box_y,p.box_z,traj.ibox,p.APS);
    sum_z.get_dim(p.box_x,p.box_y,p.box_z,traj.ibox,p.APS);
    p2.get_dim(p.box_x,p.box_y,p.box_z,traj.ibox,p.APS);
    phi.get_dim(p.box_x,p.box_y,p.box_z,traj.ibox,p.APS);
    theta.get_dim(p.box_x,p.box_y,p.box_z,traj.ibox,p.APS);
    dist.get_dim(p.box_x,p.box_y,p.box_z,traj.ibox,p.APS);
