...
void dump_grid_pdb(Trajectory &traj,system_variables &s,program_variables &p,vector<Grid> &coords_x,vector <Grid> &coords_y,
                   vector <Grid> &coords_z,vector <Grid_d> &beta,Grid_i &nan,Ref &top)
{
    ...
    FILE *mlc_file;
    mlc_file = fopen(p.mlc_file_name.c_str(), "w");
    if(mlc_file == NULL)
    {
            ...
            if(strcmp(traj.atom_name[j].c_str(), p.map_1.c_str()) == 0 || strcmp(traj.atom_name[j].c_str(), p.map_2.c_str()) == 0) //map atom 1 or 2 
            {
                ...
                if(strcmp(traj.res_name[traj.target_leaflet[i]-1].c_str(), p.target_lip.c_str()) == 0) //lipid type is correct
                {
                    for(k=0; k<top.get_num_atoms(); k++) //loop over lipid atoms
                    {
                        ...
                        coords_x[k].stamp(hx,hy,p.radius,x);
                        coords_y[k].stamp(hx,hy,p.radius,y);
                        coords_z[k].stamp(hx,hy,p.radius,z);
                    }
                }
            }
        }
                       ...
                       vector <Grid> &coords_z,Grid &rho_t,vector <Grid_d> &beta,Grid_i &nan,Ref &top)
{
    ...
    if(s.world_rank == 0)
    {
        ...
        dump_grid_pdb(traj,s,p,coords_x,coords_y,coords_z,beta,nan,top);
    }
...
int main(int argc, const char * argv[]) 
{
    ...
    add_argument_mpi_s(argc,argv,"-param",  p.param_file_name,            "Ref file with target lipid (gro, pdb)",                       s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-mlc",    p.mlc_file_name,              "Output data file with average coords (pdb)",                  s.world_rank, nullptr,      1);
    ...
    add_argument_mpi_s(argc,argv,"-m1",     p.map_1,                      "Name of mapping atom 1 ",                                     s.world_rank, nullptr,      1);
    add_argument_mpi_s(argc,argv,"-m2",     p.map_2,                      "Name of mapping atom 2 ",                                     s.world_rank, nullptr,      1);
    ...
    add_argument_mpi_i(argc,argv,"-g_strd", &p.grid_stride,               "How many grid points to skip in output pdb?",                 s.world_rank, nullptr,      0);
    add_argument_mpi_i(argc,argv,"-rho_t",  &p.b_rho_t,                   "Use rho_t for excluding data (0:no, 1:yes)",                  s.world_rank, nullptr,      0);
