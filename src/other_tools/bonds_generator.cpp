...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
//                                                                                                           //
// This function checks the bond is part of an elastic network                                               //
//                                                                                                           //
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
int check_elastic(int b_elastic_network,string spring_constant)
{
    int is_elastic = 0;
...
int main(int argc, const char * argv[]) 
{
    ...
    int b_elastic_network = 0;     //Include bonds from elastic network? 
                                    ...
                                    if(strcmp(itp_files[j][pos_line][0].c_str(), this_mol.c_str() ) == 0) //molecule type is correct
                                    {
                                        ...
                                        if(bonds_result == 1 && (next_result == 0 || bonds_line_pos < next_type_line_pos)) //[bonds] section found befor the next [moleculetype] section
                                        {
                                            ...
                                            for(k=0; k<1;)
                                            {
                                                ...
                                                {
                                                    if(itp_files[j][bonds_line_pos].size() >= 1) //line has at least 1 item
                                                    {
                                                        if(itp_files[j][bonds_line_pos][0][0] != '[') //still in current [bonds] section
                                                        {
                                                            if(itp_files[j][bonds_line_pos][0][0] != ';' && itp_files[j][bonds_line_pos][0][0] != '#' && itp_files[j][bonds_line_pos][0][0] != '\r')
                                                            {
                                                                ...
                                                                if(check_elastic(b_elastic_network,itp_files[j][bonds_line_pos][4]) == 0)
                                                                {
                                                                    these_bonds.push_back(this_bond);
                                                                }
                                                                else 
                                                                {
                                                                    printf("Ignoring rubber band: %d \n",bonds_line_pos);
                                                                }
                                                            }
                                                        }
                                        ...
                                        {                                 
                                            ...
                                            if(constraints_result == 1 && (next_result == 0 || constraints_line_pos < next_type_line_pos)) //[constraints] section found befor the next [moleculetype] section
                                            {
                                                ...
                                                for(k=0; k<1;)
                                                {
                                                    ...
                                                    {
                                                        if(itp_files[j][constraints_line_pos].size() >= 1) //line has at least 2 itmes
                                                        {
