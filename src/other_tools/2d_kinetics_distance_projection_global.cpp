...
int main(int argc, const char * argv[]) 
{
    ...
    result = events_global.get_info(in_file_name);
    int first_global   = 1;
    ...
    for(i=0; i<prot_mask.size_y(); i++) //loop over y
    {
        for(j=my_xi; j<=my_xf; j++) //loop over x
        {
            ...
            if(prot_mask.grid[j][i][2][0] == 0) //check that grid point is not at protein
            {
                ...
                //                                                                                                           //
                ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
                result = events_global.get_binding_events_xy(in_file_name,j,i);
            ...
            int current_step = i*my_num_g_x + ef_x;
            int my_steps     = my_num_g_x*prot_mask.size_y();
    ...
    if(world_rank == 0)
    {
        time_line_file_name      = add_tag(base_file_name_o, "_time_line_global.dat");
    ...
    //log time spent performing main analysis
    perf.log_time((clock() - t)/CLOCKS_PER_SEC,"Global Count");
    ...
    if(my_iterations > 0)
    {
        for(q=my_i; q<=my_f; q++) //loop over the iterations
        {
            ...
            //                                                                                                           //
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
            out_file_name = add_tag(base_file_name_o, "_" + to_string(q) + "_small_mask.dat");
